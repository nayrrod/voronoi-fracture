...
function init() {
    ...
    let vertices = getRandom2DVertices(guiParams.sites, guiParams.distribution, width, height)
...
function initGui(params) {
    ...
    f1.add(params, 'distribution', ['gaussian', 'uniform']).name('Distribution').onFinishChange(function (val) {
        scene.remove(bufferMesh)
        let vertices = getRandom2DVertices(guiParams.sites, guiParams.distribution, width, height)
        let voronoiDiagram = generateVoronoiDiagram(-width / 2, width / 2, -height / 2, height / 2, vertices)
        let shapeArray = voronoiDiagram.cells.map(getVoronoiShape)
